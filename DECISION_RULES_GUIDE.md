...
This is documented explicitly in `chargeback_analysis.py`'s own docstring, and it's the correct behavior, not a bug: unattended automation shouldn't be deciding to fight a dispute on a merchant's behalf without any evidence to back that decision — "refund" (or `None`, for a code with no rule at all) are the only two outcomes that make sense without a human in the loop, from `decision_rules.decide()` alone. `analyze_chargeback()` itself has two exceptions that run *before* ever reaching `decide()`, both of which can produce `"fight"` on their own precisely because they're checking real, database-verified evidence rather than asking the merchant for any: the CBS refund check (`cbs.py`'s `find_refund_for_utr()` — a confirmed prior refund on the exact transaction), and, for U002 specifically, a ledger-based check (`cbs.py`'s `count_credits()`/`has_pending_suspense()` — the bank's own posting records can directly confirm or refute a duplicate-transaction claim without needing the merchant to assert anything). A single clean credit is checked two further, independent ways before being trusted, not one: `cbs.py`'s `reconcile_utr()` first confirms it agrees with what the payment network independently reported as settled TO THE MERCHANT, and `cbs.py`'s `get_debit_attempt_status()` then checks the network's own transaction log for how many times the CUSTOMER's account was actually debited — a question a clean merchant-side ledger can never answer on its own, since a duplicate debit could in principle occur upstream (at a PSP/intermediary layer) and be reversed, or worse, succeed without its credit ever reaching this merchant's ledger at all. Any of the three checks disagreeing defers the decision (`action=None`) rather than trusting a partial picture; the network check disagreeing outright (a genuine, unreversed duplicate debit invisible to the merchant's ledger) flips the recommendation to `"refund"` entirely, since the customer's claim turns out to be true at the network level regardless of what the merchant's own books show. See `cbs.py`'s module docstring and `chargeback_analysis.py`'s `analyze_chargeback()` (steps 2, 2b, 2c) for the full reasoning — 2c was added after a live conversation exposed the gap: a merchant asked "what if an intermediate party charged the customer?" and the honest answer, at the time, was that the system had never actually checked.

`Analysis` also carries a `deadline_expired: bool`, orthogonal to `action`/`source` above — it doesn't change WHETHER to fight or refund, it flags whether the case's `response_deadline` has already passed, computed once via `merchant_db.deadline_bucket()` and returned from every branch (including the `action=None` ones — an expired deadline is a fact about the case, not something only a confident recommendation carries). Added after a separate live complaint: the deadline being in the past was mentioned as prose in an answer, but never changed what was actually recommended. See `CHARGEBACK_AGENT_GUIDE.md`'s sixth bug for how `chargeback_agent.py` uses this field to change guidance, not just word it differently.
