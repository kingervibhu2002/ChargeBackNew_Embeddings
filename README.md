...
- **`text_to_sql.py`** — turns a plain-English question about a merchant's chargebacks into SQL, runs it, and formats the result. Enforces role-based scoping at the code level (not just via the LLM's prompt): a `merchant` role always gets `WHERE merchant_id = <their own id>` force-injected regardless of what SQL the LLM generated or what the question asked about; admin roles are not scoped. Also enforces that only `SELECT` statements can ever run. A separate, earlier check recognizes an escalation attempt ("show me other merchants'/users'/customers' data") in plain language before SQL generation even runs, so a merchant gets a clear "you can only query your own data" message rather than the SQL-generation step failing on an ambiguous request and surfacing a confusing, unrelated-looking error — the row-level scoping itself is unconditional either way. If the underlying LLM call itself fails (e.g. a provider rate limit), the error surfaced back to the caller is a generic, merchant-safe message — never the raw provider exception, which for some providers includes internal account/billing details that have no business reaching an end user. A real, verified reliability gap: the identical question "how much is outstanding this month?", sent unchanged across separate calls, was non-deterministically generating different SQL — sometimes filtering `transaction_date`, sometimes `chargeback_filing_date`; sometimes `status='Open'` only, sometimes `status IN ('Open','Pending')` — producing genuinely different rupee totals for the same question. Fixed with explicit prompt rules pinning both down: "outstanding"-style questions with no explicit transaction verb always mean `chargeback_filing_date` (the chargeback's own timeline, not the underlying purchase's), and always mean `status='Open'` only, matching the same definition of "open"/"outstanding" used everywhere else in this app (`merchant_db.list_open_chargebacks()`, the Dispute Assistant's case-listing feature). A related but distinct bug found later: that same "always status='Open'" reasoning was over-applied by the LLM to plain COUNT questions with no financial-liability framing at all — "how many chargebacks do I currently have?" was silently answered as "how many are literally status='Open' right now" (1, instead of the real total of 18). Fixed with an explicit rule distinguishing the two: a bare count/history question ("how many chargebacks do I have," "...this year") means the total across every status, and only an explicit status word or genuine financial-exposure framing ("outstanding," "owe," "at risk") narrows it to Open. A second, unrelated formatting bug surfaced in the same trace — an unaliased aggregate column (`COUNT(*)`) rendered as the literal label "Count(*): **1**" instead of a real answer; fixed by requiring the LLM to always alias aggregate/computed columns with a plain-words name. A third instance found later, this time in `chargeback_agent.py`'s ROUTING to this module rather than the SQL generation itself: "why my chargeback is pending" non-deterministically produced three different outcomes across identical calls — the real case-listing tool (which only ever shows `status='Open'` rows, never `'Pending'`, a different real status this schema has, so it could never have surfaced the right answer even when "correctly" chosen), or a fabricated, ungrounded generic explanation from a plain knowledge-base search with no connection to the merchant's real data. Calling `query_chargebacks()` directly confirmed the underlying capability already worked correctly every time — only the LLM tool-call deciding whether to route here at all was unreliable. Fixed the same way the "outstanding" routing gap was fixed earlier: a new deterministic check (`classifier.looks_like_status_question()` — a real status word like "pending"/"expired" plus a personal reference like "my"/"our") bypasses that LLM judgment call entirely for this phrasing. Fixing the routing then surfaced a second-order issue in the SQL itself: once reliably routed here, "why is my chargeback pending" was interpreted too literally as "just the reason text," generating `SELECT reason_description ... LIMIT 1` — correct but useless, since a merchant with 2 matching cases got neither case_id nor any indication a second case existed. Fixed with an explicit rule: a "why is X `<status>`" question must select enough columns to stand on its own (case_id, reason_description, status, response_deadline at minimum) and must not add `LIMIT 1` unless something else explicitly calls for exactly one row. A fourth instance found immediately after, this time not even the SQL layer — a genuinely direct "Tell me about my chargebacks." got a completely generic, ungrounded explanation of what a chargeback is (US-centric MATCH-list/fee-range boilerplate, no connection to the merchant's real data or even this platform's RuPay/UPI domain), confirmed via a debug trace to be `chargeback_agent.py`'s own tool-calling decision (`_resolve_data_lookup_intent`) returning ZERO tool calls at all for this exact phrasing — not a wrong tool, no tool, despite `list_merchant_cases`' own docstring describing almost this exact wording. Fixed the same way: a new deterministic check (`classifier.looks_like_new_request()` — already built earlier this session for a different purpose, recognizing a fresh imperative ask; confirmed no overlap with the count/aggregate/status checks above) bypasses the LLM tool-call decision for this phrasing and routes straight to the real case list.
