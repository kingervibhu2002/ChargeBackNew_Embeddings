...
**One deliberate limitation, documented rather than silently accepted:** this conversational memory lives entirely in the browser, which resends the whole exchange on every turn (see §7's checkpointer note for why). A "complete" answer — like a fully-answered general question — resets that memory. So a case you were just discussing can drop out of context if the conversation branches into a fully-answered tangent before you return to it. This is a real, known gap, not an oversight — fixing it properly means changing how the browser tracks conversation state, a larger change than the case-conversation feature itself needed.
