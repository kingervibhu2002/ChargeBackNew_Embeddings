...
This is a real, live piece of infrastructure: `api_server.py` constructs an actual `SqliteSaver` on startup (`checkpoints.db`, in local file mode — see `.gitignore`, which excludes it and its WAL sidecar files as regenerable/runtime state) and passes it into `build_dispute_agent(checkpointer=_checkpointer)`. Every `/dispute` response includes a `thread_id`, scoped to the caller's identity (`{merchant_id}:{uuid4}` — checked on the way back in, so one merchant can't resume a session that belongs to another).

**As of a later revision, this checkpointer is no longer inert — `chat.html` now rounds-trips `thread_id` for real** (mirroring the `anchoredCaseId` pattern below): it captures `thread_id` from every response and resends it on the next call, so a second `.invoke()` genuinely resumes the same LangGraph thread instead of starting a brand-new one every time. This was safe to wire up specifically *because* of the reducer work in §2.8: `DisputeAgent.run()`'s `initial_state` still passes empty dicts for `case_context`/`conversation`/`decision_ctx` on every call (unchanged from before), but since `_merge_dict(old, {})` is a no-op (`{**old, **{}} == old`), that empty input no longer wipes a resumed thread's checkpointed state — it just leaves it alone. No `graph.get_state()`-based branching turned out to be necessary in `run()` at all; the reducer's own merge semantics did the work.

That still isn't the whole story, though. Multi-turn conversations mostly work through the *separate*, older mechanism described in the paragraphs below (`chat.html`'s `pendingQuery`/`accumulatedContext` resending), and most nodes re-derive their fields from that resent text on every single call regardless of what the checkpoint holds — because a node that runs on every turn (like `_planner_node`) always includes its freshly-computed value in its return dict, even when that value is blank, and the reducer's `{**old, **new}` merge means an explicit fresh blank *does* overwrite a checkpoint-restored one. So checkpoint restoration by itself doesn't override live re-derivation; it only helps in the gap re-derivation leaves: `_planner_node`'s case-reference fallback chain (see below) now tries the checkpoint-restored `case_context.case_id` as a third-priority source, after an explicit case reference in this turn's text and the `anchored_case_id` hint — genuine defense-in-depth for the one specific field this project has repeatedly gotten wrong, not a wholesale replacement of the resent-text architecture.

Multi-turn conversations work primarily through a *separate*, simpler mechanism: `chat.html` itself remembers the original question (`pendingQuery`, set once on turn 1) and re-sends it verbatim on every subsequent call, along with every follow-up reply joined together as `additional_context` (capped to the most recent turns rather than growing without bound). `_validate_node` re-derives whatever it needs (which case was being discussed, whether a reply resolves to a specific case selection, etc.) fresh from that resent text on every single call — this remains the primary mechanism; the checkpoint is a narrower safety net layered on top of it, not a replacement for it.

Why the resent-text mechanism matters in practice: it's *why* a case-selection sub-conversation could lose context at all — and it's exactly what broke, live, before a fix: `pendingQuery` originally got cleared the moment ANY single question in the conversation received a complete, standalone answer, even mid-topic, so a case discussed several turns earlier could vanish entirely once a fully-answered tangent came in between. Reported with a real transcript and fixed in `chat.html` — memory now survives a completed answer, clearing only on an actual topic change or an explicit reset (switching merchant/tab).

**A narrower, earlier fix for one specific symptom of the "no real memory" problem above — the client-supplied hint, distinct from the checkpoint.** Re-deriving everything from resent text is usually reliable, but confirmed live to occasionally lose track of which case is anchored: a reply shaped like a fresh imperative ask ("show me the ledger entries for this transaction") gets promoted to replace `masked_query` outright (see `classifier.looks_like_new_request()`), and if that promoted text has no case reference of its own, `_planner_node`'s case lookup comes up empty even though a real case was clearly still the subject. `DisputeRequest`/`DisputeResponse` already round-trip `case_id` every turn (the server always tells the client which case it resolved to) — `chat.html` captures it (`anchoredCaseId`) and resends it, cleared at the same three points `pendingQuery`/`threadId` now are: merchant switch, tab switch, `looksLikeNewTopic()`. `_planner_node`'s case-lookup regex uses it as a fallback — only when THIS turn's own text has no case reference, and never overriding an explicit, different case reference the merchant actually typed (checked first, unconditionally). This predates the checkpoint wiring above and stays in place alongside it, checked first since it's the cheaper, more direct of the two.

**Why a genuine topic change doesn't leak a stale case through the checkpoint.** `threadId` is cleared client-side at the exact same three points `anchoredCaseId` is. A cleared `threadId` means the next `/dispute` call supplies none, so the server mints a brand-new one with no prior checkpoint at all (`api_server.py`'s `if not thread_id: thread_id = f"{owner_tag}:{uuid.uuid4()}"`) — there's nothing to restore `case_context` from, so a new topic can't accidentally inherit an old case's identity. Confirmed live: the same vague follow-up ("What should I do about this chargeback?") resolves the previously-anchored case when replayed with its `thread_id`, and resolves to no case at all when replayed with an empty one.
...
`chargeback_agent.py` now has three such fields — `case_context`, `conversation`, `decision_ctx` — from a refactor that consolidated ~20 of `ChargebackState`'s original ~40 flat fields into structured, reducer-backed groups. Phase 1 dual-wrote (flat field and nested dict both set, nothing reading the nested copy yet); Phase 2 finished the migration — every node's *reads* go through the nested dicts (`case_ctx.get("reason_code")`, `decision_ctx.get("evidence_present")`, etc.), and the redundant flat fields (`reason_code`, `decision`, `evidence_present`, `needs_more_info`, and ~16 others) were deleted from `ChargebackState` entirely, not just left unused. Phase 4 went further and folded in the one pair of fields Phase 1-2 deliberately left alone — `decision` and the flat `case_intro_action`/`reason`/`source` (validate_node's evidence-blind CBS/ledger-only case-intro preview) were genuinely two separate representations of "what does the system recommend," kept apart because unifying them changes behavior, not just where a value lives. They're now one `decision_ctx` shape with an `origin: "main_pipeline" | "case_intro_preview"` tag — see §3's case-intro trace and `DecisionContext`'s own docstring for why the two stay mutually exclusive by construction. The one field left genuinely untouched by any of this is `is_valid_query`/`retrieved_docs`/`retrieval_status`/`retrieval_issues`/`draft_response`/`iteration`/`final_answer` (never part of the three-bucket split to begin with).
...
- **Checkpointing** — a real `SqliteSaver` is wired up, and `chat.html` now rounds-trips `thread_id` for real (§2.7) — a second `.invoke()` on the same thread genuinely resumes from the checkpoint, not just a fresh state every call. Its practical scope is narrow and deliberate: `_planner_node`'s case-reference fallback chain is the one place that reads a checkpoint-restored value, as a third-priority source after an explicit case reference and the `anchored_case_id` hint. `thread_id` still does double duty as a security-scoping token (a guessed/leaked `thread_id` can't be replayed against another merchant's identity) independent of resumption.
- **Custom reducers** — `ChargebackState` has three `Annotated[dict, _merge_dict]` fields (`case_context`, `conversation`, `decision_ctx`), with `_merge_dict` doing a plain shallow `{**old, **new}` merge instead of LangGraph's default whole-key overwrite. Confirmed via a standalone spike script (Phase 0), then exercised for real once every node's reads were migrated onto these dicts (Phase 2) and the old flat fields deleted — see §2.8. Every fight/refund decision, evidence tag, and clarification round in the graph now actually flows through this reducer, not a demo of it sitting inert. It's also what makes checkpoint resumption above safe: an empty-dict input on a resumed thread is a no-op merge, not an overwrite.
...
- **Self-correcting retry loops** — `reflect_node` scores groundedness and confidence but has only one outgoing edge, straight to `END`. A low-confidence or ungrounded result is reported, never fed back into another retrieval/generation attempt. `planner_node` now does compute a real retrieval-sufficiency SIGNAL (`retrieval_evaluator.evaluate_retrieval()` — checks whether the retrieved documents' own `network` payload field actually matches the network just detected for the query, not just whether they scored well on similarity) and writes it to `retrieval_status`/`retrieval_issues` — but nothing yet routes on it or retries retrieval when it comes back "bad." That's a deliberate first step (an evaluator with no corrective action wired to it yet), not the same thing as a correction loop.
  ...
  **A sharper limitation, confirmed live, worth stating precisely**: `reflect_node` isn't just non-correcting — it's also not universal. It sits on exactly one edge, `generate → reflect → END`, the full dispute-decision pipeline. Several other answer-producing paths never reach it at all: the case-selection/rich-intro shortcut and the mid-dispute clarifying-question path (`_answer_clarification_node → ask_user → END`) both return a final answer with zero groundedness/confidence verification applied. This isn't hypothetical — it's exactly how a real bug reached a merchant: an out-of-range case reference ("what about the third one?" when only two cases existed) got silently re-grounded in the wrong (first) case by `_validate_node`'s continuity logic, and nothing downstream caught it, because nothing downstream ever ran. Worth a second, separate note: even if `reflect_node` *did* cover every path, its specific check (does a cited reason code trace back to real docs/rules) wouldn't have caught this particular error anyway — U002 genuinely is a real code, just for the wrong case. `reflect_node` verifies code grounding, not conversational-reference correctness; the two are different problems, and fixing this one required a fix in `classifier.py`/`_validate_node`, not in `reflect_node`.
...
| Checkpointer | Save/resume state across separate `.invoke()` calls, keyed by `thread_id` | Real `SqliteSaver` wired in `api_server.py` (`checkpoints.db`); `chat.html` rounds-trips `thread_id` so a resumed thread's checkpoint is genuinely restored (see §2.7) |
