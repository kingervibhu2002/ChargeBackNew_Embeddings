...
Multi-turn conversations work anyway, through a *completely separate*, simpler mechanism: `chat.html` itself remembers the original question (`pendingQuery`, set once on turn 1) and re-sends it verbatim on every subsequent call, along with every follow-up reply joined together as `additional_context` (capped to the most recent turns rather than growing without bound). `_validate_node` re-derives whatever it needs (which case was being discussed, whether a reply resolves to a specific case selection, etc.) fresh from that resent text on every single call — there is no persisted memory across calls at all, LangGraph-native or otherwise; it's entirely re-derivation from resent text.

Why this matters in practice: it's *why* a case-selection sub-conversation could lose context at all — and it's exactly what broke, live, before a fix: `pendingQuery` originally got cleared the moment ANY single question in the conversation received a complete, standalone answer, even mid-topic, so a case discussed several turns earlier could vanish entirely once a fully-answered tangent came in between. Reported with a real transcript and fixed in `chat.html` — memory now survives a completed answer, clearing only on an actual topic change or an explicit reset (switching merchant/tab). The underlying architecture point stands regardless of that fix: nothing is ever remembered server-side; everything downstream is re-derived fresh from whatever text the browser currently happens to be resending, so the browser's own bookkeeping is the entire memory system, checkpointer or not.
