...
```bash
pip install pytest
pytest test_classifier.py test_decision_rules.py test_chunking.py -q   # pure-Python, no external deps
...
- `evidence_tags.py` — the closed `EvidenceTag` vocabulary both of the above depend on (kept in its own module to avoid a circular import between `chargeback_agent.py` and `decision_rules.py`). Distinguishes independently-verified evidence (e.g. `single_credit_confirmed` — a bank statement/ledger/CBS record cited) from a bare, unverified merchant claim (`single_credit_self_reported`) — the latter can never independently satisfy a `decision_rules.RULES` entry, so a merchant's own say-so about their own side of a transaction is never treated as proof of the customer's side.
- `chunking.py` also derives a **chunk-level knowledge taxonomy** the same rule-based way: `derive_knowledge_type()` classifies a chunk's `##`/`###` heading text into a fixed vocabulary (`DEFINITION`, `EVIDENCE`, `PROCEDURE`, `RESPONSIBILITY`, `OUTCOME`, `DEADLINE`, `EXCEPTION`, `FAQ`, `SCENARIO`, `REBUTTAL`, `SUMMARY`) via keyword match (vocabulary chosen from the corpus's actual recurring headings, e.g. `grep -rh '^## ' chargeback-encyclopedia/`); `derive_actors()`/`derive_evidence_tags()` scan chunk *content* for role mentions (merchant/customer/issuer/acquirer/psp/network) and for the existing `evidence_tags.EvidenceTag` vocabulary, respectively. `classifier.py`'s `detect_knowledge_type_intent()`/`detect_actor_intent()` are the query-side counterpart, mapping question phrasing to the same vocabularies. `_answer_question_node`'s precision Q&A path (`VectorStore.search_chunks()`'s `knowledge_type` filter) tries a filtered lookup first and falls back to the unfiltered pool on empty results — this is what lets "what does U002 mean?" and "what if the PSP charged the customer twice?" retrieve different chunks instead of both landing on the same undifferentiated similarity ranking within a network/code. `actors`/`evidence_tags` are lower-confidence (content-derived, not heading-derived) and are used only as a client-side tie-break within an already-fetched pool, not a hard Qdrant filter.
- `chargeback_agent.py`'s `_derive_assessment()` layers a 5-value `assessment` (`INVESTIGATE` / `CONTEST` / `ACCEPT` / `INSUFFICIENT_EVIDENCE` / `NO_ACTION_AVAILABLE`) on top of the fight/refund `decision` above, computed once in `run()` from fields every node already populates — never a graph-state field itself. `CONTEST`/`ACCEPT` mirror `decision`; `INVESTIGATE` covers a genuine bank/network-side blocker or reconciliation that was simply never checked (`chargeback_analysis.py`'s U002 handling walks the ledger → network-reconciliation chain for exactly this before ever defaulting to a bare rule-table guess). `decision` itself is kept as-is for chat.html UI backward-compat; `assessment`/`recommendation`/`case_status`/`resolution_status`/`deadline_status` are additive fields on `DisputeResponse`.
...
### Data stores
- `chargebacks.db` (SQLite) — `chargebacks` table (NPCI/UPI dispute records, seeded by `merchant_db.py`) + `usermaster` table (login identities, seeded by `usermaster.py`) + `cbs.py`'s `ledger_entries`/`settlement_entries`/`network_debit_attempts` tables (simulated bank/network records `chargeback_analysis.py` reconciles against). `cbs.ensure_investigate_demo_case()` idempotently adds one fixed demo case (`NPCI20260818M002777`) with a reconciled ledger credit but no network reconciliation at all, so the INVESTIGATE assessment always has real data to exercise — called from `api_server.py` startup and `cbs.py`'s own `__main__` reseed.
